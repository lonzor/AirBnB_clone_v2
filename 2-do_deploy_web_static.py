...
def do_deploy(archive_path):
    ...
    try:
        ...
        run("sudo tar -xzvf /tmp/" + archiveName + " -C " + "/data/web_static/releases/" + archNameNoExt + " --strip-components=1")
        run("sudo rm -f /tmp/" + archiveName)
        run("sudo rm -f /data/web_static/current")
        run("sudo ln -sf /data/web_static/releases/" + archNameNoExt + "/data/web_static/current")
        return True
    except:
        return False
